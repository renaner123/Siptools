...
class Arguments():
    ...
    def add_arguments(self):
        ...
        self.parser.add_argument('-user',
                                 ...
                                 help='Seconds to wait for a response.  '
                                      'Default 1s.')
        self.parser.add_argument('-callid',
                                 dest='callid',
                                 default=123456,
                                 help='Seconds to wait for a response.  '
                                      'Default 1s.')
        self.parser.add_argument('-seq',
                                 dest='seq',
                                 default=1,
                                 help='Seconds to wait for a response.  '
                                      'Default 1s.')
        self.parser.add_argument('-mac',
                                 dest='mac',
                                 default="3C0754399E3D",
                                 help='Seconds to wait for a response.  '
                                      'Default 1s.')
        self.parser.add_argument('-model',
                                 dest='model',
                                 default="Algum Modelo",
                                 help='Seconds to wait for a response.  '
                                      'Default 1s.')
        self.parser.add_argument('-sip_from',
                                 dest='sip_from',
                                 default="123",
                                 help='Seconds to wait for a response.  '
                                      'Default 1s.')
        self.parser.add_argument('-sip_to',
                                 dest='sip_to',
                                 default="456",
                                 help='Seconds to wait for a response.  '
                                      'Default 1s.')
        self.parser.add_argument('-rtp_port',
                                 dest='rtp_port',
                                 default="6000",
                                 help='Seconds to wait for a response.  '
                                      'Default 1s.')
...
def main():
    try:
        ...
        strImprimir = request.bytes.decode();
        stringformatada = strImprimir % {'dest_ip': args.dest_addr, 'source_ip': args.src_ip,
                                         'source_port': args.src_port, 'user': args.user,
                                         'dest_port': args.dest_port, 'callid': args.callid, 'seq': args.seq,
                                         'mac': args.mac,'model':args.model, 'sip_from':args.sip_from, 'sip_to':args.sip_to,
                                         'rtp_port':args.rtp_port, 'apelido':args.a, 'expire':args.expire}
        stringbytes = stringformatada.encode()
        s.send(stringbytes)
        ...
        if not args.quiet:
            ...
            if args.verbose:
                ...
                stringformatada = strImprimir % {'dest_ip': args.dest_addr, 'source_ip': args.src_ip,
                                                ...
                                                'mac': args.mac,'model':args.model, 'sip_from':args.sip_from, 'sip_to':args.sip_to,
                                                'rtp_port':args.rtp_port, 'apelido':args.a, 'expire':args.expire}
                #sys.stdout.write(stringformatada + "\n")
        ...
        sys.stdout.write("Response from %s:%d\n\n" %
                            (args.dest_addr, args.dest_port))
        if args.verbose:
            #strImprimirresposta = request.bytes.decode();
